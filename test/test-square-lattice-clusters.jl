...
function E0_dimer_collinear(J, s)
    return -J[1] / 4 - J[1] * s^2 + 4 * J[2] * s^2 -
           sqrt(J[1]^2 + 4 * (J[1] - 4 * J[2])^2 * s^2) / 2
end

@testset "Dimer cluster in collinear phase" begin
    uc = UnitCell(; lattice_vecs=[[1.0, 0.0], [0.0, 1.0]], basis_vecs=[[0.0, 0.0]])
