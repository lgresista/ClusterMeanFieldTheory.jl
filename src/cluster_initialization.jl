...
# run fixedpoint iterations and save results to file
function run_cmft(
    J,
    uc,
    bonds,
    ...
    outpath;
    spinoperators = calculate_spinoperators(get_nsites(uc, L)),
    max_iterations=1000,
    abstol=1e-8,
    β=0.5,
    max_error_increase_count=max_iterations,
    verbose=false,
)

    mfcluster = get_meanfield_cluster(uc, bonds, J, L)
    set_magnetizations!(mfcluster, initial_magnetizations)
    recalculate_magnetic_fields!(mfcluster)

    # start iterative solver
    is_converged, iterations, abserror = fixedpoint_iteration!(
        mfcluster;
        ...
        abstol=abstol,
        β=β,
        max_error_increase_count=max_error_increase_count,
    )

    h5open(outpath, "w") do f
        f["e0"] = calculate_groundstate_energy(mfcluster)
        f["magnetizations"] = stack(mfcluster.magnetizations)
        f["iterations"] = iterations
        f["is_converged"] = is_converged
        f["abserrors"] = abserror

        f["J"] = J
        attributes(f)["beta"] = β
        attributes(f)["abstol"] = abstol
        attributes(f)["max_iterations"] = max_iterations
        attributes(f)["L"] = L
    end
end

# run fixed point iterations for set of Js,
function run_batch_cmft(
    Js,
    uc,
    bonds,
    L,
    get_initial_magnetizations,
    get_outpath;
    max_iterations=1000,
    abstol=1e-8,
    β=0.5,
    ...
    N = get_nsites(uc, L)
    spinoperators = calculate_spinoperators(N)
    
    for J in Js
        initial_magnetizations = get_initial_magnetizations(J)
        outpath = get_outpath(J)

        @repeat 4 try # in case of LAPACKException retry 4 times
            run_cmft(
                J,
                uc,
                bonds,
            ...
            @retry if e isa LAPACKException 
                verbose && println("LAPACKException occured for J = $J, retrying.")
            end
        end
    end
end